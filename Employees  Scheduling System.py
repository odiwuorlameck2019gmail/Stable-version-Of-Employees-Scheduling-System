...
def Create_Record():
    ...
    insert_data=(first_name1_.get(),second_name1_.get(),third_name1_.get(),id_number1_.get(),work_id2_.get(),phone_number1_.get(),gender1_.get(),job_title1_.get(),
...
def Employee_Pie_Chart_Analysis_():
     ...
     cursor.execute(" SELECT DISTINCT count(*) FROM employees_absent WHERE work_id={}".format(work_id1_.get()))
...
def Male_Employees_Absent():
    ...
    while(i<arrival_no):
        ...
        if(t.year==YEAR and t.month==MONTH and t.day==DAY):
                     ...
                     for Present  in _Absent:
                         if Present not in _present:
                              if(str(Present) not in str(work_id_for_employees_present_today)):
                                    cursor.execute("INSERT INTO employees_absent(work_id) VALUES({})".format(Present))
                                    conn.commit()
...
labelframe.grid(column=200,row=0)
show_employees=tk.Button(labelframe,text="Apologetic Employees",font='yellow',bg='blue',width=30,command=List_Employees_Absent_with_apologies)
